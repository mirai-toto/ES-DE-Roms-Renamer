...
def rename_main(filename, system, perform_rename, config_path):
    ...
    correct_filename = get_correct_filename(invalid_filename, config_path)
...
if __name__ == "__main__":
    if len(sys.argv) not in [4, 5]:  # Allow 4 or 5 arguments
        print("Usage: python rename_main.py <filename> <system> <perform_rename> [<config_path>]")
        sys.exit(1)

    filename = sys.argv[1]
    system = sys.argv[2]
    perform_rename = sys.argv[3].lower() in ['true', '1', 'yes']
    config_path = sys.argv[4] if len(sys.argv) == 5 else CONFIG_PATH
